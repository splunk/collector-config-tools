...
class C3FieldsetMapFormController {

  constructor(sb, cfgSchema, multiFieldsetView, onSubmit) {
    console.log('C3FieldsetMapFormController', cfgSchema);
    this.sb = sb;
    this.userInputs = [];
    this.cfgSchema = cfgSchema;
    this.onSubmit = onSubmit;
    this.rootView = new C3FieldsetFormView(cfgSchema["name"], sb);
  ...
  addField() {
    const fieldView = new C3FieldView(this.sb, "map key");
    ...
    if (this.mapValueType === 'string') {
      const fieldView = new C3FieldView(this.sb, "map value");
      fieldView.appendInputWidget(new TextInputWidget())
      this.rootView.appendToFormView(fieldView);
    } else {
      const cfgSchema = {
